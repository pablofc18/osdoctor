...
pub fn emit(results: &mut Vec<CheckResult>, scope: Scope, q: &Query) {
    ...
    match q.status {
        QueryStatus::Ok if q.count <= 0 => {
            results.push(CheckResult::new(id, GROUP, CheckStatus::Ok, format!("No failed {noun} services"), ""));
        }
        QueryStatus::Ok => {
        ...
        QueryStatus::Unavailable => {
            results.push(CheckResult::new(id, GROUP, CheckStatus::Skip, "systemctl not available", ""));
        }
    }
}
...
fn query_systemctl(scope: Scope) -> Query {
    ...
    if scope == Scope::User && code != Some(0) {
        return Query { status: QueryStatus::NoSession, count: 0, detail: String::new() };
    }
    let (count, detail) = count_units(&out);
    Query { status: QueryStatus::Ok, count, detail }
}
...
mod tests {
    ...
    fn emit_ok_zero() {
        let mut r = Vec::new();
        emit(&mut r, Scope::System, &Query { status: QueryStatus::Ok, count: 0, detail: String::new() });
        ...
        emit(&mut r, Scope::User, &Query { status: QueryStatus::Ok, count: 0, detail: String::new() });
    ...
    fn emit_ok_counts_and_plural() {
        let mut r = Vec::new();
        emit(&mut r, Scope::System, &Query { status: QueryStatus::Ok, count: 3, detail: "a, b, c".into() });
        assert_eq!(last(&r).status, CheckStatus::Warn);
        assert_eq!(last(&r).message, "3 failed system services");
        assert_eq!(last(&r).detail, "a, b, c");

        emit(&mut r, Scope::System, &Query { status: QueryStatus::Ok, count: 1, detail: "only".into() });
        assert_eq!(last(&r).message, "1 failed system service");

        emit(&mut r, Scope::User, &Query { status: QueryStatus::Ok, count: 1, detail: "only".into() });
    ...
    fn emit_no_session_user() {
        let mut r = Vec::new();
        emit(&mut r, Scope::User, &Query { status: QueryStatus::NoSession, count: 0, detail: String::new() });
    ...
    fn emit_unavailable() {
        let mut r = Vec::new();
        emit(&mut r, Scope::System, &Query { status: QueryStatus::Unavailable, count: 0, detail: String::new() });
        emit(&mut r, Scope::User, &Query { status: QueryStatus::Unavailable, count: 0, detail: String::new() });
        assert!(r.iter().all(|c| c.status == CheckStatus::Skip && c.message == "systemctl not available"));
    }
